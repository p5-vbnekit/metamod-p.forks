...
def scan(api: str, key: str, ref: str, repo: str, token: str):
    ...
    while True:
        ...
        if not _data:
            if 0 < _page: break
            if _bogus_ref: break
            if not ref.startswith("refs/tags/"): break
            _bogus_ref = f"refs/heads/{ref}"
            print(
                f"no one cache entry found, replacing the {ref} whith {_bogus_ref}"
                " to work around possible api bug", file = sys.stderr, flush = True
            )
            _total = None
            ref = _bogus_ref
            continue
...
def query(api: str, key: str, token: str, version: str):
    ...
    _response = requests.post(
        f"{api}/twirp/github.actions.results.api.v1.CacheService/GetCacheEntryDownloadURL",
        data = json.dumps(dict(key = key, version = version)),
        headers = {
    ...
    assert isinstance(_data, dict)
    assert _data["ok"] is True
    assert _data["matched_key"] == key
    _url =  _data["signed_download_url"]
