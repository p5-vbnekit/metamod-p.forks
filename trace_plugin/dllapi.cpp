...
int DispatchSpawn(edict_t *entity) {
    DLL_TRACE(pfnSpawn, P_PRE, (
        "classname=%s", entity ? STRING(entity->v.classname) : "nil"
    ));
    // 0==Success, -1==Failure ?
    RETURN_META_VALUE(MRES_IGNORED, 0);
}
void DispatchThink(edict_t *) {
    DLL_TRACE(pfnThink, P_PRE, (""));
    RETURN_META(MRES_IGNORED);
}
void DispatchUse(edict_t */*used*/, edict_t */*other*/) {
...
void DispatchBlocked(edict_t */*blocked*/, edict_t */*other*/) {
    DLL_TRACE(pfnBlocked, P_PRE, (""));
    RETURN_META(MRES_IGNORED);
}
void DispatchKeyValue(edict_t *, KeyValueData *kvd) {
    DLL_TRACE(pfnKeyValue, P_PRE, (
        "classname=%s keyname=%s value=%s", P_PRE,
        kvd->szClassName, kvd->szKeyName, kvd->szValue
    ));
    RETURN_META(MRES_IGNORED);
}
void DispatchSave(edict_t *, SAVERESTOREDATA *) {
    DLL_TRACE(pfnSave, P_PRE, (""));
    RETURN_META(MRES_IGNORED);
}
int DispatchRestore(edict_t *, SAVERESTOREDATA *, int /*global_entity*/) {
    DLL_TRACE(pfnRestore, P_PRE, (""));
    // 0==Success, -1==Failure ?
    RETURN_META_VALUE(MRES_IGNORED, 0);
}
void DispatchObjectCollsionBox(edict_t *entity) {
    DLL_TRACE(pfnSetAbsBox, P_PRE, (
...
BOOL ClientConnect(
    ...
    (void)reject_reason;
    DLL_TRACE(pfnClientConnect, P_PRE, ("name=%s, addr=%s", name, address));
    RETURN_META_VALUE(MRES_IGNORED, FALSE);
}
void ClientDisconnect(edict_t *entity) {
    // DLL_TRACE(pfnClientDisconnect, P_PRE, ("name=%s", ENTITY_KEYVALUE(pEntity, "name")));
    DLL_TRACE(pfnClientDisconnect, P_PRE, ("name=%s", STRING(entity->v.netname)));
    RETURN_META(MRES_IGNORED);
}
void ClientKill(edict_t *entity) {
    DLL_TRACE(pfnClientKill, P_PRE, ("name=%s", STRING(entity->v.netname)));
    RETURN_META(MRES_IGNORED);
}
void ClientPutInServer(edict_t *entity) {
    DLL_TRACE(pfnClientPutInServer, P_PRE, ("name=%s", STRING(entity->v.netname)));
    RETURN_META(MRES_IGNORED);
}
void ClientCommand(edict_t *entity) {
    DLL_TRACE(pfnClientCommand, P_PRE, (
        "name=%s, cmd='%s %s'",
        STRING(entity->v.netname), CMD_ARGV(0),
        CMD_ARGC() >= 1 ? CMD_ARGS() : ""
    ));
    RETURN_META(MRES_IGNORED);
}
void ClientUserInfoChanged(edict_t *entity, char */*info_buffer*/) {
    DLL_TRACE(pfnClientUserInfoChanged, P_PRE, ("name=%s", STRING(entity->v.netname)));
    RETURN_META(MRES_IGNORED);
}
void ServerActivate(edict_t */*list*/, int count, int client_max) {
    DLL_TRACE(pfnServerActivate, P_PRE, ("count=%d, max=%d", count, client_max));
    RETURN_META(MRES_IGNORED);
}
...
void PlayerPreThink(edict_t *entity) {
    DLL_TRACE(pfnPlayerPreThink, P_PRE, ("name=%s", STRING(entity->v.netname)));
    RETURN_META(MRES_IGNORED);
}
void PlayerPostThink(edict_t *entity) {
    DLL_TRACE(pfnPlayerPostThink, P_PRE, ("name=%s", STRING(entity->v.netname)));
    RETURN_META(MRES_IGNORED);
}
...
void PlayerCustomization(edict_t *entity, customization_t *) {
    DLL_TRACE(pfnPlayerCustomization, P_PRE, ("name=%s", STRING(entity->v.netname)));
    RETURN_META(MRES_IGNORED);
}
void SpectatorConnect(edict_t *) {
    DLL_TRACE(pfnSpectatorConnect, P_PRE, (""));
    RETURN_META(MRES_IGNORED);
}
void SpectatorDisconnect(edict_t *) {
    DLL_TRACE(pfnSpectatorDisconnect, P_PRE, (""));
    RETURN_META(MRES_IGNORED);
}
void SpectatorThink(edict_t *) {
    DLL_TRACE(pfnSpectatorThink, P_PRE, (""));
    RETURN_META(MRES_IGNORED);
}
void Sys_Error(const char *error) {
    DLL_TRACE(pfnSys_Error, P_PRE, ("string=%s", error));
    RETURN_META(MRES_IGNORED);
}

// from SDK pm_shared/pm_shared.c:
void PM_Move(struct playermove_s *, int /*server*/) {
...
void UpdateClientData(const struct edict_s *, int /*send_weapons*/, struct clientdata_s *) {
...
void CreateBaseline(
    ...
    struct entity_state_s *, struct edict_s *,
...
int GetWeaponData(struct edict_s *, struct weapon_data_s *) {
...
int GetHullBounds(int /*hull_number*/, float */*mins*/, float */*maxs*/) {
...
int InconsistentFile(const edict_t *player, const char *file_name, char */*disconnect_message*/) {
    DLL_TRACE(pfnInconsistentFile, P_PRE, ("name=%s, file=%s", STRING(player->v.netname), file_name));
    RETURN_META_VALUE(MRES_IGNORED, 0);
}
...
// from SDK ?
void OnFreeEntPrivateData(edict_t *) {
...
void CvarValue(const edict_t *entity, const char *value) {
    NEWDLL_TRACE(pfnCvarValue, P_PRE, (
        "player=%s, value=%s",
...
void CvarValue2(const edict_t *entity, int request_id, const char *name, const char *value) {
    NEWDLL_TRACE(pfnCvarValue2, P_PRE, (
        "player=%s, requestID=%d, cvar=%s, value=%s",
        STRING(entity->v.netname), request_id,
        name ? name : "nil",
        value ? value : "nil"
    ));
    RETURN_META(MRES_IGNORED);
...
static DLL_FUNCTIONS global_functions = {
...
C_DLLEXPORT int GetEntityAPI2(DLL_FUNCTIONS *functions, int *interface_version) {
    LOG_DEVELOPER(PLID, "called: GetEntityAPI2; version=%d", *interface_version);
    if(!functions) {
    ...
    if(*interface_version != INTERFACE_VERSION) {
        LOG_ERROR(
            PLID, "GetEntityAPI2 version mismatch; requested=%d ours=%d",
    ...
    memcpy(functions, &global_functions, sizeof(DLL_FUNCTIONS));
    return TRUE;
}
...
static NEW_DLL_FUNCTIONS global_new_functions = {
...
C_DLLEXPORT int GetNewDLLFunctions(NEW_DLL_FUNCTIONS *functions, int *interface_version) {
    LOG_DEVELOPER(PLID, "called: GetNewDLLFunctions; version=%d", *interface_version);
    if(!functions) {
    ...
    if(*interface_version != NEW_DLL_FUNCTIONS_VERSION) {
        LOG_ERROR(
            PLID, "GetNewDLLFunctions version mismatch; requested=%d ours=%d",
    ...
    memcpy(functions, &global_new_functions, sizeof(NEW_DLL_FUNCTIONS));
    return TRUE;
}
