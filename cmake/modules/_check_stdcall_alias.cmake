function("_check_stdcall_alias" "_output_variable")
    if(":${_output_variable}:" STREQUAL "::")
        message(FATAL_ERROR "_output_variable is empty")
    endif()

    unset("_result" CACHE)
    get_property("_result" DIRECTORY "${CMAKE_SOURCE_DIR}" PROPERTY "_check_stdcall_alias.result" SET)
    mark_as_advanced("_result")

    if (_result)
        get_property("_result" DIRECTORY "${CMAKE_SOURCE_DIR}" PROPERTY "_check_stdcall_alias.result")
        if(":${_result}:" STREQUAL "::")
            message(FATAL_ERROR "${CMAKE_SOURCE_DIR}: _check_stdcall_alias.result property is empty")
        endif()
    else()
        ...
        try_compile(
            "_result"
            "${_compile_time_tests_directories.build}/stdcall_alias"
            "${_compile_time_tests_directories.source}/stdcall_alias"
            "metamod-p.compile_time_tests.stdcall_alias"
        )

        set_property(
            DIRECTORY "${CMAKE_SOURCE_DIR}"
            PROPERTY "_check_stdcall_alias.result"
            "${_result}"
        )
    endif()
