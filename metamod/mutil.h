...
typedef struct meta_util_funcs_s {
    void            (*pfnLogConsole)        (plid_t plid, char const *fmt, ...);
    void            (*pfnLogMessage)        (plid_t plid, char const *fmt, ...);
    void            (*pfnLogError)          (plid_t plid, char const *fmt, ...);
    void            (*pfnLogDeveloper)      (plid_t plid, char const *fmt, ...);
    void            (*pfnCenterSay)         (plid_t plid, char const *fmt, ...);
    void            (*pfnCenterSayParms)    (plid_t plid, hudtextparms_t tparms, char const *fmt, ...);
    void            (*pfnCenterSayVarargs)  (plid_t plid, hudtextparms_t tparms, char const *fmt, va_list ap);
    qboolean        (*pfnCallGameEntity)    (plid_t plid, char const *entStr, entvars_t *pev);
    int             (*pfnGetUserMsgID)      (plid_t plid, char const *msgname, int *size);
    char const *    (*pfnGetUserMsgName)    (plid_t plid, int msgid, int *size);
    char const *    (*pfnGetPluginPath)     (plid_t plid);
    char const *    (*pfnGetGameInfo)       (plid_t plid, ginfo_t tag);

    int (*pfnLoadPlugin)(plid_t plid, char const *cmdline, PLUG_LOADTIME now, void **plugin_handle);
    int (*pfnUnloadPlugin)(plid_t plid, char const *cmdline, PLUG_LOADTIME now, PL_UNLOAD_REASON reason);
    int (*pfnUnloadPluginByHandle)(plid_t plid, void *plugin_handle, PLUG_LOADTIME now, PL_UNLOAD_REASON reason);

    char const *    (*pfnIsQueryingClientCvar)  (plid_t plid, edict_t const *player);
...
DLLHIDDEN extern mutil_funcs_t MetaUtilFunctions;
